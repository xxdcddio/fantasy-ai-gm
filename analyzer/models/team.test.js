...
assert.strictEqual(team.players.length, 27);
...
assert.strictEqual(team.getIL().length, 4);
...
assert.deepStrictEqual(ss, ["Corey Seager", "Jeremy Peña", "Willi Castro"]);

// findPlayer: case/whitespace-insensitive, null when absent
assert.strictEqual(team.findPlayer("Jeremy Peña").name, "Jeremy Peña");
assert.strictEqual(team.findPlayer("  jeremy peña  ").name, "Jeremy Peña");
